...
async function import_scenario_one_from_csv() {
    for (let counter = 3; counter < 57; counter++) {
        const data_csv = fs.readFileSync(`./Data/Scenario1n/S_1_${counter}.csv`).toString().split('\n');
        data_csv.shift();
        ...
        for (const row of data_csv) {
            ...
            const car_spawned = row_data[14].trim() === 'True' ? true : false;
            const ped_spawned = row_data[15].trim() === 'True' ? true : false;
            const ped_collide = parseFloat(row_data[16].trim()) > 0 ? true : false;
            const car_collide = row_data[17].trim() === 'True' ? true : false;
        ...
        console.log('inserted full csv file', counter)
    }
}
